...
### Steps to Reproduce
<!--
If this project's configs/<KEY>.json marks a "Steps to Reproduce" field
as fieldType: "adf", write this as a real ADF orderedList — not plaintext
with literal "1." "2." — one listItem per step, and set it as that field's
value (not inline in description). One concrete action per step. Include an
exact URL/path/query wherever one exists so someone can copy-paste and go.
After any step with a visible result, add a bold "Expected:" step.
-->
...
### Acceptance Criteria
<!--
REQUIRED — never omit. Plain bullet list, not numbered. Each bullet is one
testable, unambiguous condition a QA person can check off with no extra
context. No open questions, no "confirm with X" — those go in Overview or a
separate ticket.
-->
- {testable condition}
- {testable condition}
